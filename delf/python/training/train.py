...
flags.DEFINE_string('logdir', 'weights', 'WithTensorBoard logdir.')
flags.DEFINE_string('train_file_pattern', 'tfrecord/train*',
                    'File pattern of training dataset files.')
flags.DEFINE_string('validation_file_pattern', 'tfrecord/validation*',
                    'File pattern of validation dataset files.')
flags.DEFINE_enum(
    'dataset_version', 'gld_v2', ['gld_v1', 'gld_v2', 'gld_v2_clean'],
  ...
  with strategy.scope():
    ...
    with summary_writer.as_default():
      with tf.summary.record_if(
        ...
        file_path = '%s/variables' % FLAGS.logdir
